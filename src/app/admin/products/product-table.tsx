...
export function DataTableToolbar<TData>({ table }: DataTableToolbarProps<TData>) {
  ...
  return (
    <div className="flex gap-1 dark:text-white items-center">
          ...
          <Input className="max-w-xs" placeholder="stock filter..."
              value={(table.getColumn("stock")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("stock")?.setFilterValue(event.target.value)}                
          />
          
      
          <Input className="max-w-xs" placeholder="pedidoEnOrigen filter..."
              value={(table.getColumn("pedidoEnOrigen")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("pedidoEnOrigen")?.setFilterValue(event.target.value)}                
          />
          
      
          <Input className="max-w-xs" placeholder="precioUSD filter..."
              value={(table.getColumn("precioUSD")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("precioUSD")?.setFilterValue(event.target.value)}                
          />
