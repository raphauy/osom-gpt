...
export function DataTableToolbar<TData>({ table }: DataTableToolbarProps<TData>) {
  ...
  return (
    <div className="flex items-center gap-1 dark:text-white">
      ...
      {table.getColumn("clasificacion") && (
        <DataTableFacetedFilter
          column={table.getColumn("clasificacion")}
          title="Clasificación"
          options={["General", "Industrial", "Comercial", "Residencial"]}
        />
      )}

      <Input className="max-w-xs" placeholder="idTrackeo filter..."
          value={(table.getColumn("idTrackeo")?.getFilterValue() as string) ?? ""}
